...
var global_obj={
	...
	share_layer:function(remove){
		...
		$('#global_share_layer').css({
			...
			width:'100%',
			height:'100%',
			background:'url(./Public/static/images/layer.png) left top no-repeat',
	...
	upload_images:function(obj){
		var i=0,localIds='';
		$(obj+' .add').click(function(){
			chooseImage();
		});
		files_count=$(obj).find('input[name=ImgsQty]').val();
		...
		uploadImage=function(){
			...
			wx.uploadImage({localId:localIds[i],success:function(res){
				$('<div class="items del"><span></span><img src="'+localIds[i]+'" /><input type="hidden" name="ServerId[]" value="'+res.serverId+'"/></div>').insertBefore(obj+' .add');
				$(obj+' .del').off('click').on('click',function(){
					$(this).remove();
					imageCount();
				});
				imageCount();
				i++;
				if(i<localIds.length && i<files_count){uploadImage();}
			}});
		}
		imageCount=function(){
			if($(obj+' .del').size()>=files_count){
				$(obj+' .add').hide();
				return;
			}else{ $(obj+' .add').show();}	
		}
