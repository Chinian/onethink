...
var businesscard_obj={
	...
	businesscard_init:function(){
		...
		businesscard_obj.show_code(); //输出二维码
		businesscard_obj.code_close();
		businesscard_obj.ctrl_music();
	},
	ctrl_music:function(){
		var oMc=$('#mc');		
		var isMusic=true;
		...
		$(document).bind('touchstart',function(){
			if(oMc.length>0){
				if(isMusic){
					oMc.get(0).play();
					$('#music img').attr('src','./Public/Businesscard/images/off.png');
					isMusic=false;
				}
			}
		});
		$('#music img').click(function(){
			if(playing){
				oMc.get(0).pause();
				$('#music img').attr('src','./Public/Businesscard/images/on.png');
				playing=false;
			}else{
				oMc.get(0).play();
				$('#music img').attr('src','./Public/Businesscard/images/off.png');
