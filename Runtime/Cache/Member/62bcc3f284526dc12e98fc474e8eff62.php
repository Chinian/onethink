...
<script type='text/javascript' src='/test/onethink/Public/member/js/app_hotels.js?t=20150325' ></script>

	<script type="text/javascript" src="/test/onethink/Public/static/plugin/daterangepicker/moment.min.js"></script>

	<link rel="stylesheet" type="text/css" href="/test/onethink/Public/static/plugin/daterangepicker/daterangepicker.css" media="all">
	...
	<div class="control_btn"><a href="http://weixin.jucheng01.com/member/?m=app_hotels&amp;a=reserve&amp;d=edit" class="btn_ok btn_ok_w_120">预订设置</a></div>
	<form class="r_con_search_form" id="search_form" method="get" action="<?php echo U();?>?">
		...
		<input name="c" value="Hotel" type="hidden">
	</form>
	<table class="r_con_table" border="0" cellpadding="5" cellspacing="0">
		<thead>
			<tr>
				<td nowrap="nowrap" width="8%">序号</td>
				<td nowrap="nowrap" width="12%">联系人</td>
				<td nowrap="nowrap" width="10%">联系手机</td>
				<td nowrap="nowrap" width="12%">入住日期</td>
				<td nowrap="nowrap" width="10%">入住时间</td>
				<td nowrap="nowrap" width="12%">离店日期</td>
				<td nowrap="nowrap" width="10%">离店时间</td>
				<td nowrap="nowrap" width="12%">预订房型</td>
				<td nowrap="nowrap" width="10%">提交时间</td>
				<td nowrap="nowrap" width="10%">订单状态</td>
				<td class="last" width="10%">操作</td>
			</tr>
		</thead>
		<tbody>
		<?php if(is_array($data["reserve"])): $i = 0; $__LIST__ = $data["reserve"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
				<td nowrap="nowrap"><?php echo ($val["id"]); ?></td>
				<td><?php echo ($val["name"]); ?></td>
				<td><?php echo ($val["phone"]); ?></td>
				<td><?php echo (date("Y-m-d",$val["checkin_time"])); ?></td>
				<td><?php echo (date("H:i",$val["checkin_time"])); ?></td>
				<td><?php echo (date("Y-m-d",$val["departure_time"])); ?></td>
				<td><?php echo (date("H:i",$val["departure_time"])); ?></td>
				<td><?php echo ($val["pname"]); ?></td>
				<td nowrap="nowrap"><?php echo (date("Y-m-d H:i",$val["inputtime"])); ?></td>
				<td nowrap="nowrap"><?php echo ($val["status"]); ?></td>
				<td class="last" nowrap="nowrap"><a href="<?php echo U('hotel/reservedetail', array('id' => $val['id']));?>">
				<img src="/test/onethink/Public/Member/images/view.gif" alt="预订查看" align="absmiddle"></a>
				<a href="http://weixin.jucheng01.com/member/?m=app_hotels&amp;a=reserve&amp;do_action=app_hotels.reserve_list_del&amp;LId=63" onclick="if(!confirm('删除后不可恢复，继续吗？')){return false};">
				<img src="/test/onethink/Public/Member/images/del.gif" alt="删除" align="absmiddle">
				</a>
				</td>
