...
<div class="r_nav">
	<ul>
		<li class="cur"><a href="<?php echo U('wedding/index');?>">信息管理</a></li>
		<li class=""><a href="<?php echo U('wedding/index');?>">信息推荐</a></li>
		<li class=""><a href="<?php echo U('wedding/catview');?>">栏目编辑</a></li>
	</ul>
</div>
	...
	<table class="r_con_table" border="0" cellpadding="5" cellspacing="0">
		<thead>
			<tr>
				...
				<td nowrap="nowrap" width="24%">主题</td>
				<td nowrap="nowrap" width="12%">发布时间</td>
				<td nowrap="nowrap" width="12%">过期时间</td>
				<td nowrap="nowrap" width="12%">地区</td>
				<td nowrap="nowrap" width="10%">图片数量</td>
				<td nowrap="nowrap" width="10%">审核状态</td>
				<td class="last" width="14%">操作</td>
			</tr>
		</thead>
		<tbody>
		<?php if(is_array($data["list"])): $i = 0; $__LIST__ = $data["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
				...
				<td nowrap="nowrap"><?php echo ($val["category"]); ?></td>
				<td nowrap="nowrap"><?php echo ($val["title"]); ?></td>
				<td nowrap="nowrap"><?php echo (date("Y-m-d H:m:s",$val["inputtime"])); ?></td>
				<td nowrap="nowrap"><?php echo (date("Y-m-d H:m:s",$val["expirytime"])); ?></td>
				<td nowrap="nowrap"><?php echo ($val["region"]); ?></td>
				<td nowrap="nowrap"><?php echo ($val["pic"]); ?></td>
				<td nowrap="nowrap">
				<input type="checkbox" <?php if($val["status"] ): ?>checked="checked"<?php endif; ?> data-id="<?php echo ($val["id"]); ?>" data-toggle="switch" id="custom-switch-03" onchange="checkinfo(this)"/>
				</td>
				<td class="last" nowrap="nowrap">
				<a href="<?php echo U('wedding/infoview', array('id' => $val['id']));?>" title="查看">
				<img src="/test/onethink/Public/Member/images/view.gif"/>
				</a>
				<a href="<?php echo U('wedding/del', array('infid' => $val['id']));?>" title="删除" onclick="if(!confirm('删除后不可恢复，继续吗？')){return false};">
				<img src="/test/onethink/Public/Member/images/del.gif"/>
				</a>
				</td>
