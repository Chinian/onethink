...
<div id="banner_box" class="box_swipe">
	<ul>
		<?php if(is_array($config["index"]["banner_path"])): foreach($config["index"]["banner_path"] as $k=>$v): ?><li><img src="<?php echo ($v); ?>" alt="<?php echo ($k + 1); ?>" style="width:100%;" /></li><?php endforeach; endif; ?>

	</ul>
	<ol>
		<?php if(is_array($config["index"]["banner_path"])): foreach($config["index"]["banner_path"] as $k=>$v): if($k == 0): ?><li class="on"></li>
		<?php else: ?>
		<li></li><?php endif; endforeach; endif; ?>
	</ol>
</div>
<script type="text/javascript">app_car_obj.app_car_banner_swipe_init();</script>
<nav>
<div class="nav_list"><a href="<?php echo U('Car/product');?>">车型</a></div>
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="nav_list"><a href="<?php echo U('Car/news', array('cid' => $v['id']));?>"><?php echo ($v["name"]); ?></a></div><?php endforeach; endif; else: echo "" ;endif; ?>
<div class="nav_list"><a href="<?php echo U('Car/reserve');?>"><?php echo ($config["reserve"]["rename_title"]); ?></a></div>

<div class="nav_list"><a href="<?php echo U('Car/owner');?>">车主关怀</a></div>
<div class="nav_list"><a href="<?php echo U('Car/about');?>">关于我们</a></div>
<div class="nav_list"><a href="<?php echo U('Car/tools');?>">实用小工具</a></div>
<div class="nav_list"><a href="<?php echo U('Car/contact');?>">联系销售</a></div>
